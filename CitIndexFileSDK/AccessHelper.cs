...
namespace CitIndexFileSDK
{
    ...
    {
        //数据库连接字符串
        public static readonly string conn = string.Empty;
        // 用于缓存参数的HASH表
        ...
        /// <returns></returns>
        public static int CheckAndCreateTable(string connectionString,string tableName,string cmdText,params OleDbParameter[] commandParameters)
        {
            //创建一个SqlCommand对象
            OleDbCommand cmd = new OleDbCommand();
            //创建一个SqlConnection对象
            ...
            try
            {
                ...
                PrepareCommand(cmd, conn, null, cmdText, commandParameters);
                //调用conn判断表是否存在
                bool isExist = conn.GetSchema("Tables", new string[4] { null, null, tableName, "TABLE" }).Rows.Count > 0;
                //不存在则创建
                if (!isExist)
                {
